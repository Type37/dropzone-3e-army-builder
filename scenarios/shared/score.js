...
.ss-l{font-size:14px;color:#5d5850;margin-right:8px;}
.ss-seg button{font:600 16px/1 'Jost',system-ui,sans-serif;min-width:34px;height:32px;padding:0 10px;border:0;background:#f1ece2;color:#3d3834;cursor:pointer;}
...
.ss-h{padding:16px 0 2px;font:600 20px/1.3 'Jost',system-ui,sans-serif;}
.ss-h mark{color:inherit;background:linear-gradient(transparent 45%,#f1dc92 45%,#f1dc92 92%,transparent 92%);padding:0 4px;margin-left:-4px;}
.ss-h span{font:400 14px 'Jost',system-ui,sans-serif;color:#5d5850;margin-left:8px;}
...
@media print{.ss{display:none !important;}}`;
  ...
  function mount(el, cfg) {
    ...
    function draw() {
      ...
      const rowHTML = r => {
        if (r.heading) return `<li class="ss-h"><mark>${r.heading}</mark>${r.sub ? `<span>${r.sub}</span>` : ''}</li>`;
        const parts = r.parts || [];
