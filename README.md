...
### 2026-08-01 — A run spent looking at it
- **The print preview was lying to a phone.** The same one-page army read "1 page" at desktop width and "3 pages" at 375, with a break line drawn through the middle of a Squad — through a Group, which is the one atom the whole sheet is built to keep whole. Two faults, both invisible on a desktop: the scroll box is a flex container whose default is `align-items: stretch`, so the paper was pulled down to the height of the window and then measured; and `paginate()` was mixing `getBoundingClientRect`, which follows the sheet's zoom down, with `scrollHeight` and written-back styles, which do not. Measured before and after rather than reasoned about, and checked again against a generated twelve-Group army where the break has to land in a real gap.
- **On a phone there was no way back.** `.topbar-context` — the back chevron on every view, and Play Mode's Reset game — has been hidden below 768px since the days of the separate `mobile/` build. That build is deleted; nobody went back over what the deletion left behind. The chevron stays at every width now and only the label gives way, with a house rule and a test behind it.
- **Status Tokens go on the Squad**, which 11.1.7, 11.1.22 and 11.1.34 each say outright. Concussed, Suppressed and Jammed were on every model, so a Squad of five had five of each and you could record a state the game cannot produce. Obscured stays per model, because 10.1.21 makes it a question of where a model is standing rather than a token placed on a Squad. A saved game is migrated up rather than dropped. A status you have set now spells its whole name; an unset one stays a letter, because a legend line under the row would be the caption under a control this app does not write.
- **Three things were being drawn over or cut off.** Every Group card printed its cost with the last digits under the drag grip. The Unit Reference cut the ends off its own stats — `5+ DEFENC`, `5+ BRAVE` — because a bare `1fr` is `minmax(auto, 1fr)` and the label refuses to break inside a word. The Collection's search box cut off the line that says what you can type; on a phone the magnifier gives way instead of the words.
- **Category spend drew Standard's track in the category green, on a card whose background is the faction accent.** On UCM those are the same green, so the one row that explains what the other three mean read as a smudge beside a number.
- **An unnamed Commander said its Level twice**, in the rail and on the printed sheet, because the derived name already is the Level. The unit page did the same with the unit's name. **An empty Group** was tagged "orphaned transports" in Play Mode and told a rule about transports it did not contain.
- **Every screenshot ever taken of this app was of its 0% keyframe.** Headless Chrome does not advance the animation timeline when nothing is asking it for frames, and `.stagger > *` starts at `opacity: 0` — so the landing page captured as one grey ghost on an empty page, in every set anyone has taken. `scripts/shots.mjs` finishes every animation before the capture, and both committed sets have been retaken. It also shoots a second Group, the rail, a reference card, Share and Play, none of which had ever been shot.
