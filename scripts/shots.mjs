...
/* SHOT_DSF renders at a device pixel ratio, and SHOT_FIT then scales the file
 * down to an exact size. Both exist for the Open Graph card and are off by
 * default, because every other shot in here is a 1:1 legibility check and
 * capturing those at 2x re-rasterises text crisper than anyone will ever see
 * it (see the note by the clip code).
 *
 * The card is the opposite problem. It has to be exactly 1200x630 and it has to
 * survive being shown 400px wide in a Discord embed, so it is laid out at
 * 1120x588 -- same aspect, fewer CSS pixels, everything proportionally larger
 * -- shot at 2x and fitted back down. The old card was a 1600x840 grab whose
 * 13px stat labels came out as grey fuzz, which is the entire reason this
 * exists. */
const DSF = Number(process.env.SHOT_DSF) || 1;
const FIT = (process.env.SHOT_FIT || '').split('x').map(Number).filter((n) => n > 0);
 ...
 *
 *   SHOT_W=1120 SHOT_H=588 SHOT_DSF=2 SHOT_FIT=1200x630 \
 *     node scripts/shots.mjs <base> assets/logos og
 *
 * The army is removed again afterwards: this runs against a real profile and
 * has no business leaving a list in anyone's browser storage.
 */
const OG_STEPS = [
  ['og-preview', `
     await DZC.loadFaction('ucm');
     const A = DZCArmy;
     (A.all() || []).filter(x => x.name === 'Reconquest of Earth')
       .forEach(x => A.remove(x.id));
     const a = A.create('ucm', 'Reconquest of Earth', 1500);
     const tanks = A.addSquad(a, A.addGroup(a).id, 'ucm-main-battle-tank', 3);
     A.assignTransport(a, tanks.id, 'condor-dropship');
     const leg = A.addSquad(a, A.addGroup(a).id, 'legionnaires', 3);
     A.assignTransport(a, leg.id, 'bear-apc');
     const arch = A.addSquad(a, A.addGroup(a).id, 'archangel', 2);
     A.toggleUpgrade(a, arch.id, '*', 'UM-115 Missile Spread');
     const mbt = A.addSquad(a, A.addGroup(a).id, 'ucm-main-battle-tank', 2);
     A.setCommander(a, mbt.id, 5);
     window.__ogArmy = a.id;
     location.hash = '#army/' + a.id;
     await new Promise(r => setTimeout(r, 1600));
     // Open a Group, or the right-hand pane is empty and the card is 40%
     // background. The one with the Transport in it, because a Squad nested
     // under the thing carrying it is the part of this builder worth showing.
     const first = document.querySelector('.dzc-bb');
     if (first) first.click();
     await new Promise(r => setTimeout(r, 900));
     // The description placeholder reads "What is this army for?" on an army
     // that has no description, which is a prompt to the person building it and
     // not something to advertise.
     const d = document.querySelector('.dzc-b-desc');
     if (d && !d.textContent.trim()) d.remove();
     // Scroll the open Group up under the top bar.
     //
     // Unscrolled, this layout puts the Group cards in a row, the Add Group
     // button under them, and the opened Group's contents below that -- which
     // at card proportions means a third of the picture is empty paper between
     // the two. Scrolling to the detail panel spends those pixels on the
     // Condor with its Squad nested underneath and a stat card open, which is
     // the part of this builder worth advertising. The top bar is sticky, so
     // the wordmark stays.
     const panel = document.querySelector('.dzc-gpanel, .dzc-group-detail')
       || [...document.querySelectorAll('*')].find((e) => /^Group 1/.test(e.textContent || '')
            && e.getBoundingClientRect().top > 300);
     if (panel) {
       const bar = document.querySelector('.topbar, .app-bar, header');
       const off = bar ? bar.getBoundingClientRect().height : 48;
       window.scrollTo(0, window.scrollY + panel.getBoundingClientRect().top - off - 10);
     }
     await new Promise(r => setTimeout(r, 500));
  `],
];
...
await s.send('Emulation.setDeviceMetricsOverride', { ...VIEW, deviceScaleFactor: DSF, mobile: false });
...
for (const [name, expr, clipSel] of STEPS) {
  ...
  if (FIT.length === 2) {
    /* Pillow rather than a node image library: scripts/ already assumes Python
       for the art and data work, and this does not justify a dependency. */
    execFileSync('python', ['-c', `
from PIL import Image
im = Image.open(r"${file}")
if im.size != (${FIT[0]}, ${FIT[1]}):
    im.convert("RGB").resize((${FIT[0]}, ${FIT[1]}), Image.LANCZOS).save(r"${file}", "PNG", optimize=True)
`], { stdio: 'inherit' });
  }
  done.push(name);
  console.log(`  ${file}`);
